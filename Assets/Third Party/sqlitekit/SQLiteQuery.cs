...
public class SQLiteQuery {
	...
	{
		if( Sqlite3.sqlite3_bind_text( vm, bindIndex++, str, -1, null ) != Sqlite3.SQLITE_OK ) 
		{ 
	...
	{
		if( Sqlite3.sqlite3_bind_int( vm, bindIndex++, integer ) != Sqlite3.SQLITE_OK ) 
		{ 
			throw new Exception( "SQLite fail to bind integer with error: " + Sqlite3.sqlite3_errmsg(db) ); 
		};
	}

	public void Bind( double real )
	{
		if( Sqlite3.sqlite3_bind_double( vm, bindIndex++, real ) != Sqlite3.SQLITE_OK ) 
		{ 
	...
	{
		if( Sqlite3.sqlite3_bind_blob( vm, bindIndex++, blob, blob.Length, null ) != Sqlite3.SQLITE_OK ) 
		{ 
	...
	{
		if( Sqlite3.sqlite3_bind_null( vm, bindIndex++ ) != Sqlite3.SQLITE_OK ) 
		{ 
